  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center pt-17">
      ...
      <motion.div
        ...
        initial="hidden"
        whileInView="show"
        viewport={{ once: false, amount: 0.7 }} className="grid grid-cols-1 md:grid-cols-12 gap-35 -ml-93 mt-35 max-w-6xl px-4">
        {Details.map((detail, index) => (
          <motion.div
            variants={fadeIn("right", 0)}
            initial="hidden"
            whileInView="show"
            viewport={{ once: false, amount: 0.7 }} key={index} className={`relative col-span-4 h-[450px] w-[365px] p-6 rounded-3xl shadow-2xl drop-shadow-[0_35px_35px_rgba(0,0,0,0.3)] ${detail.color} flex flex-col items-center text-center hover:scale-130 transition-all duration-300 ease-in-out`}>
            <img src={detail.image} alt="Billiard Ball" className="absolute -top-44 hover:scale-110 transition-all duration-300 ease-in-out w-50 h-85 object-cover" />
